...
export const findAll = async (req) => {
    ...
    const orders = await Orders.findAll({...where,include: [{ model: Products, required: true ,where:{ 
        [Op.or]: {
            nama_produk: {[Op.like]: `%${search}%`}
        }
    }, include: ImageProducts },{ model: Users,include:[UsersDetails] }], paranoid ,limit, offset, order: [["id","DESC"]]})   
...
export const cancel = async req => {
    ...
    const orders = await Orders.findOne({ where: { transactionId }, include:[Payments], paranoid: false})
    if(!orders) throw CreateErrorMessage("Tidak ada data",404)
    
    await sequelize.transaction(async transaction => {
        await orders.update({ status: "cancel" },{ transaction })
        await orders.Payment.update({ status: "cancel" },{ transaction })
    })
    return { 
        status:  200,
        message: `Pesanan berhasil dicancel`, 
        response: { orders: orders.reload()  } 
    }
}
...
export const findAll = async (req) => {
    ...
    const whereCount = { where: { status: "selesai", deletedAt: { [(paranoid) ? Op.is : Op.not] : null } } , paranoid: false}
    const orders = await Orders.findAll({...where,include: [{ model: Products, required: true, where:{ 
        [Op.or]: {
            nama_produk: {[Op.like]: `%${search}%`}
        }
    }, include: ImageProducts },{ model: Users,attributes:{exclude:["password","token","resetToken","resetTokenExp"]}, include:[{ model: UsersDetails }]}], paranoid ,limit, offset, order: [["id","DESC"]]})   

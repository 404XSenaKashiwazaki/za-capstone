...
import Table from '../partials/rateit/Table'
...
import { useFindAllProductRateItQuery } from '../features/api/apiRateItSlice'
...
const Packaged = ({ site }) => {
  ...
  const selectPerpage = [
    ...
    const selector = useSelector(state=>state.rateIt)
    const { dataUser } = useSelector(state=> state.auth)
    const { isRestore, message } = selector
    const storagePerpage = localStorage.getItem("cms_perpage_rateit") || 5
    ...
    const handleClikPaginate = (op) => {
        ...
        setCheckedId(null)
        setCheckedAll(false)
        localStorage.setItem("cms_page_rateit",selected + 1)
    }
    ...
    return (
        <div className="col-span-full xl:col-span-6 bg-white shadow-lg rounded-sm border border-slate-200">
            ...
            { showRating && <RatingModal id={id} itemContact={itemContact} setShowModal={setShowRating} showModal={showRating} /> }
            <TableHeader 
            title={ <span className="text-md"><FontAwesomeIcon  icon={faStar}/> Selesai</span> }
            type="noButton"
            pageName="rateit"
            search={search}

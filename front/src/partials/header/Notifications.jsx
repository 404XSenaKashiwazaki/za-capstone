...
function Notifications() {
  ...
  const { data: dataContact,isError, isLoading, error, refetch  } = useFindAllContact_Query({ search:"", page:1, perPage:5 })
    ...
    <div className="relative inline-flex ml-3">
      ...
      >
        ...
        <svg className="w-4 h-4" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
         ...
         { dataContact?.response?.contacts.length != 0 &&  <div className="absolute top-0 right-0 w-2.5 h-2.5 bg-rose-500 border-2 border-white rounded-full"></div> }
      </button>
      ...
      >
        ...
        >
          ...
          <ul>
            {  dataContact?.response?.contacts && dataContact.response.contacts.map(e=> {
              return(
                
                  <li key={e.id} className="border-b border-slate-200 last:border-0">
                    <Link
                      className="block py-2 px-4 hover:bg-slate-50"
                      to="#0"
                      onClick={() => setDropdownOpen(!dropdownOpen)}
                    >
                      <span className="block text-sm mb-2"><FontAwesomeIcon icon={faMessage} /> <span className="font-medium text-slate-800"></span> { e?.content?.slice(0,10) }...</span>
                      <span className="block text-xs font-medium text-slate-400">{ new Date(e.createdAt).toString().slice(3,25) }</span>
                    </Link>
                  </li>

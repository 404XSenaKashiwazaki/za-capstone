...
import { faReply, faSearch, faSyncAlt, faTrashAlt } from '@fortawesome/free-solid-svg-icons'
...
function Table({ 
  ...
  const handleDestroy = (e,title,id) => {
    ...
    mySwal.fire({
      ...
      text: "Anda akan menghapus contacts dari: "+title+"?",
      icon: 'warning',
                  ...
                  return (
                    <tr key={d.id}>
                      ...
                      <td className="p-2 whitespace-nowrap">
                        <div className="text-left font-medium ">{ d.username }</div>
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <div className="text-left font-medium ">{ d.email }</div>
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <div className="text-left font-medium ">{ d.content?.slice(0,20) }..</div>
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <div className="text-lg text-center p-1">
                            ...
                            <button 
                                onClick={(e) => handleResponse(e,d.id)}
                                className='
                                bg-sky-800 px-4 py-1 rounded-sm 
                                border-0 text-slate-300  w-auto 
                                h-auto text-sm  
                                font-medium text-center hover:bg-sky-900
                                hover:text-slate-200 mr-1'
                                ><FontAwesomeIcon icon={faReply} /> Tanggapi</button>
                            <button 
                              onClick={(e) => handleDestroy(e, d.username,d.id)}
                              className='

...
import { useFindOneProductsNotYetPaidQuery } from '../../features/api/apiNotYetPaid'

const Detail = ({ id, setId, showModal, setShowModal, username  }) => {
  const [ data, setData ] = useState(null)
  const { data: dataPayment  } = useFindOneProductsNotYetPaidQuery({ produkid: id, username }, { skip: (id) ? false : true })

  useEffect(() => {
    if(dataPayment?.response?.orders) setData({
      ...dataPayment.response.orders,
      pembeli: dataPayment.response.orders.Order.User.namaDepan +" "+ dataPayment.response.orders.Order.User.namaBelakang,
      email: dataPayment.response.orders.Order.User.email,
      noHp: dataPayment.response.orders.Order.User.UsersDetail.noHp,
      alamat: dataPayment.response.orders.Order.User.UsersDetail.alamat,
      provinsi: dataPayment.response.orders.Order.User.UsersDetail.provinsi,
      kota: dataPayment.response.orders.Order.User.UsersDetail.kota
    })
  },[dataPayment])
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between gap-2 px-4 h-auto mb-2"> 
        ...
        <div className=" p-4 h-auto mb-7">
          <div className="mb-10">
            ...
            <div className="w-full mt-2">
              { data?.Order.Products.map(e=> <Link to={`/products/${e.slug}`} key={e.id} className={`cursor-pointer flex justify-between gap-2  mb-4 border-t border-gray-300`}>
